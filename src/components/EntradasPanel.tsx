  ...
  return (
    <div className="bg-[#0f0f1a] border border-[#2a2a4a] rounded-lg p-3 mb-2">
      <input
        ...
        placeholder="Nome do tipo (ex: Qualidade)"
...
function DraggableEntrada({ item, projectId, accentColor, types, activeDragType }: {
  item: Item
  projectId: string
  accentColor: string
  types: ItemType[]
  ...
  const { updateEntrada, deleteEntrada } = useApp()
  const [deleteTarget, setDeleteTarget] = useState(false)
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({
    id: `entrada-${item.id}`,
    data: { type: 'entrada', itemId: item.id, projectId },
  })
  ...
  return (
    <div ref={setNodeRef}>
      <ItemCard
        ...
        onUpdate={(name, role, typeId) => updateEntrada(projectId, item.id, name, role, typeId)}
        onDelete={() => setDeleteTarget(true)}
      />
      {activeDragType === null && deleteTarget && (
        <ConfirmModal
          message={`Excluir entrada "${item.name}"?`}
          onConfirm={() => { deleteEntrada(projectId, item.id); setDeleteTarget(false) }}
...
export default function EntradasPanel({ projectId, activeDragType, collapsed, onToggle }: Props) {
  const { projects, addEntrada, addEntradaType, updateEntradaType, deleteEntradaType } = useApp()
  ...
  function handleAdd() {
    ...
    addEntrada(projectId, newName.trim(), newRole.trim() || undefined, newTypeId || undefined)
    setNewName('')
    setNewRole('')
    setNewTypeId('')
    setAdding(false)
  ...
  function accentFor(item: Item) {
    const t = project.entradaTypes.find(t => t.id === item.typeId)
    return t ? `[${t.color}]` : DEFAULT_COLOR
  }

  const grouped = project.entradaTypes.map(t => ({
    type: t,
    items: project.entradas.filter(e => e.typeId === t.id),
  }))
  const ungrouped = project.entradas.filter(e => !e.typeId || !project.entradaTypes.find(t => t.id === e.typeId))
  const hasTypes = project.entradaTypes.length > 0
  ...
  if (collapsed) {
    return (
      <div className="flex flex-col items-center py-4 h-full gap-3">
        <button
          onClick={onToggle}
          className="w-8 h-8 flex items-center justify-center text-[#3ecf8e]/60 hover:text-[#3ecf8e] hover:bg-[#3ecf8e]/10 rounded-lg transition-all text-base"
          title="Expandir Entradas"
        >›</button>
        <span className="text-[#3ecf8e]/20 text-[9px] font-bold tracking-widest select-none" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>ENTRADAS</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full p-4">
      ...
      <div className="flex items-center justify-between mb-3 shrink-0">
        <div>
          <h2 className="text-[#3ecf8e] text-xs font-bold tracking-widest uppercase">Entradas</h2>
          <p className="text-slate-600 text-[10px] mt-0.5">Arraste para vincular</p>
        </div>
        <div className="flex gap-1">
          <button
            onClick={() => setAddingType(true)}
            className="flex items-center gap-1 px-2 py-1 rounded-md text-[10px] text-[#3ecf8e] bg-[#3ecf8e]/10 hover:bg-[#3ecf8e]/20 border border-[#3ecf8e]/30 hover:border-[#3ecf8e]/60 transition-all font-semibold"
            title="Novo tipo de entrada"
          >
            <Tag size={11} />
            <span>+</span>
          </button>
          <button
            ...
            className="w-7 h-7 rounded-full bg-[#3ecf8e]/10 hover:bg-[#3ecf8e]/20 text-[#3ecf8e] flex items-center justify-center text-lg leading-none transition-colors"
            title="Adicionar entrada"
          >+</button>
          <button
            onClick={onToggle}
            className="w-7 h-7 flex items-center justify-center text-slate-600 hover:text-[#3ecf8e] hover:bg-[#3ecf8e]/10 rounded-full transition-all text-sm"
      ...
      <div className="flex-1 overflow-y-auto pr-0.5">
        ...
        {addingType && (
          <TypeForm
            onSave={(name, color) => { addEntradaType(projectId, name, color); setAddingType(false) }}
        ...
        {editingType && (
          <TypeForm
            initial={editingType}
            onSave={(name, color) => { updateEntradaType(projectId, editingType.id, name, color); setEditingType(null) }}
        ...
        {/* Add item form */}
        {adding && (
          <div className="bg-[#0f0f1a] border border-[#3ecf8e]/40 rounded-lg p-3 mb-2">
            <input
              autoFocus
              value={newName}
            ...
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter') handleAdd(); if (e.key === 'Escape') setAdding(false) }}
              placeholder="Fornecedor (ex: CLIENTE)"
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={handleAdd} disabled={!newName.trim()} className="text-xs bg-[#3ecf8e] hover:bg-[#2db87a] disabled:opacity-40 text-[#0f0f1a] font-medium px-3 py-1 rounded transition-colors">Adicionar</button>
            </div>
          </div>
        )}
        ...
        {project.entradas.length === 0 && !adding && (
          <p className="text-slate-700 text-xs text-center py-8">Nenhuma entrada ainda</p>
        )}
      </div>
      ...
      {deleteTypeTarget && (
        <ConfirmModal
          ...
          onConfirm={() => { deleteEntradaType(projectId, deleteTypeTarget.id); setDeleteTypeTarget(null) }}
